...
(function ($) {
    ...
    $(document).ready(function ($) {
        ...
        function dataTakenFromMatchSheet(dataMining) {
            ...
            teamOneName = dataMining
                .parents(".single-t-match")
                .find("[data-team-one='data-team-one']")
                .text();
            teamTwoName = dataMining
                .parents(".single-t-match")
                .find("[data-team-two='data-team-two']")
                .text();
            teamOneLogo = dataMining
                .parents(".single-t-match")
                .find(".single-team")
                .first()
                .find(".team-icon img")
                .attr("src");
            teamTwoLogo = dataMining
                .parents(".single-t-match")
                .find(".single-team")
                .last()
                .find(".team-icon img")
                .attr("src");
            teamOneScore = dataMining
                .parents(".single-t-match")
                .find(".single-team")
                .first()
                .find(".team-score")
                .text();
            teamTwoScore = dataMining
                .parents(".single-t-match")
                .find(".single-team")
                .last()
                .find(".team-score")
        ...
        function dataSetToModal() {
            ...
            $(".placing-bet-modal")
                .find(".selected-match .single-team")
                .first()
                .find(".team-name")
                .text(teamOneName);
            $(".placing-bet-modal")
            ...
            $(".placing-bet-modal")
                ...
                .first()
                .find(".team-icon img")
                .attr("src", teamOneLogo);
            $(".placing-bet-modal")
                .find(".selected-match .single-team")
                .last()
                .find(".team-icon img")
                .attr("src", teamTwoLogo);
            $(".placing-bet-modal")
                .find(".scrores-line")
                .find(".team-a-scr")
                .text(teamOneScore);
            ...
            $(".placing-bet-modal")
                .find(".pb-calc")
                .find(".total-return .ret-num")
                .text(betRatio);
            $(".placing-bet-modal")
                .find(".pb-calc")
                .find(".stake-line .stk-num")
                .text(stakeDG);
        }
        ...
        $(myModalEl).on("hidden.bs.modal", function (event) {
            if (
                $(".placing-bet")
                    .find(".single-bet-place.current-clicked-item")
                    .hasClass("already-placed")
            ) {
                $(".placing-bet")
                    .find(".single-bet-place.current-clicked-item")
                    .removeClass("current-clicked-item already-placed");
            } else {
                $(".placing-bet")
                    .find(".single-bet-place.current-clicked-item")
                    .removeClass("placed current-clicked-item");
            }
            stakeToEqual();
        ...
        $(".placing-bet")
            .find(".single-bet-place")
            .find(".team-name")
            .attr("data-team-name", "data-team-name");
        $(".placing-bet")
            .find(".single-bet-place")
            .find(".bet-ratio")
            .attr("data-bet-ratio", "data-bet-ratio");
        $(".single-t-match").each(function () {
            $(this)
                ...
                .first()
                .find(".team-name")
                .attr("data-team-one", "data-team-one");
            $(this)
                .find(".playing-teams")
                .find(".single-team")
                .filter(":nth-child(2)")
                .find(".team-name")
                .attr("data-team-two", "data-team-two");
        });
            ...
            .on("click", function (e) {
                ...
                if (!$(this).hasClass("placed")) {
                    ...
                    $(".placing-bet-modal")
                        .find(".pb-calc")
                        .find(".stake-line .stk-num")
                        .text(parseInt(for_upadte_stake.text()));
                    $(".placing-bet-modal")
                        .find(".pb-calc")
                        .find(".total-return .ret-num")
                        .text(update_betReturnDec);

                    var updatedStakeDG = (stakeDG = parseInt(for_upadte_stake.text()));
                    $(".placing-bet-modal").find(".stake-digit").text(updatedStakeDG);
                    updateplacedID = $(".placing-bet")
                        .find(".placed.current-clicked-item")
                        .attr("id");
                }
                ...
                $("body")
                    .find(".flying-shadow")
                    .css({ left: positionLeft, top: positionTop });
            });
        ...
        function scrollbaleCardResponsive() {
            if ($(window).width() > 1399) {
                ...
                if (
                    $(".singleBS").parents(".all-bs-card").height() >
                    mobileAllBScardHeight
                ) {
                    $(".singleBS").parents(".all-bs-card").overlayScrollbars({});
                } else {
                    $(".singleBS")
                        .parents(".all-bs-card")
                        .css("height", mobileAllBScardHeight);
                }
                if (
                    $(".singleBS").parents(".all-bs-card").height() > mobileAllComboHeight
                ) {
                    $(".card-combo").parents(".all-bs-card").overlayScrollbars({});
                }
                if (
        ...
        $("#bsCategory-profile")
            .find(".part-slip-header")
            .nextUntil(".sibling-4")
            .addBack()
            .wrapAll('<div class="mobile-wrapper"></div>');
        $("#bsCategory-contact")
            .find(".part-slip-header")
            .nextUntil(".sibling-4")
            .addBack()
        ...
        $(window).on("resize", function () {
            if ($(window).width() < 768) {
                $(".single-bs-card.card-combo")
                    .closest(".part-slip-header")
                    .css({ height: "auto", flex: "1" });
                $(".single-bs-card.card-combo")
                    .closest(".mobile-wrapper")
                    .css({ display: "flex", "flex-direction": "column" });

                $(".single-bs-card.card-system")
                    .closest(".part-slip-header")
                    .css({ height: "auto", flex: "1" });
                $(".single-bs-card.card-system")
                    .closest(".mobile-wrapper")
        ...
        function newBScardAppend() {
            ...
            var markedElementBS = $(".part-slip-header")
                .find(".all-bs-card")
                .find(".single-bs-card.singleBS.hidden")
                .parent();
            var markedElementCombo = $(".part-slip-header")
                .find(".all-bs-card")
                .find(".single-bs-card.card-combo.hidden")
                .parent();
            var markedElementSystem = $(".part-slip-header")
                .find(".all-bs-card")
                .find(".single-bs-card.card-system.hidden")
                .parent();
        ...
        function totalStakeCount() {
            ...
            $(".bet-slip-calculation")
                .find(".total-est-return")
                .text(tatalEstReturn.toFixed(2));
        }

        function slipCounter() {
            var bs_single_length = $(".single-bs-card.singleBS").not(
                ".hidden",
            ).length;
        ...
        $("#keepInSlip").on("click", function () {
            if (modalOpenFromPage == true) {
                if (
                    !$(".single-bet-place.current-clicked-item").hasClass(
                        "already-placed",
                    )
                ) {
                    $(".placing-bet")
                    ...
                    // update/edit function
                    var updatePlaced_id = $(".placing-bet")
                        .find(".placed.current-clicked-item")
                        .attr("id");
                    $('[data-match-id="' + updatePlaced_id + '"]')
                        .find(".stake-number")
                        .text(stakeDG);
                    $('[data-match-id="' + updatePlaced_id + '"]')
                        .find(".return-amount > .number")
                        .text(bettotal);
                    totalStakeCount();
                    displayEmptySlip();
                    $(".successfull-card").hide();
                ...
                if (
                    $(".single-bet-place.current-clicked-item")
                        .parents(".single-t-match")
                        .hasClass("match-in-play")
                ) {
                    inPlayIconAddToSlipCard(
                        $(".single-bet-place.current-clicked-item").attr("id"),
                    );
                    if ($(".single-bet-place.current-clicked-item").hasClass("up")) {
                        ...
                        $(".single-bet-place.current-clicked-item").hasClass("down")
                    ) {
                        inPlayAddToSlipCard(
                            $(".single-bet-place.current-clicked-item").attr("id"),
                            "#ef6665",
                ...
                $(".placing-bet")
                    .find(".placed.current-clicked-item")
                    .removeClass("current-clicked-item");
            } else {
                $('[data-match-id="' + clicked_place_id + '"]')
                    .find(".stake-number")
                    .text(stakeDG);
                $('[data-match-id="' + clicked_place_id + '"]')
                    .find(".return-amount > .number")
            ...
            if (mobileAllBScardOpen == false) {
                ...
                mobileAllComboHeight = $(".card-combo")
                    .parents(".all-bs-card")
                    .height();
            }
            ...
            var positionLeft = elementLeft - viewportLeft;
            console.log(
                "Element position: (" + positionLeft + ", " + positionTop + ")",
            );
        ...
        $(document).on("click", ".slip-dlt", function () {
            ...
            deleting_match_id = $(this)
                .parents(".single-bs-card")
                .attr("data-match-id");
            similarBSCardRemove();
        ...
        $(document).on(
            "click",
            '.bs-card-footer button[data-bs-toggle="modal"]',
            function () {
                var modal_match_id = $(this)
                    .parents(".single-bs-card")
                    .attr("data-match-id");
                var placedSelector = $(
                    '.single-bet-place.placed[id="' + modal_match_id + '"]',
                );
                ...
                $(".placing-bet-modal")
                    .find(".pb-calc")
                    .find(".total-return .ret-num")
                    .text(updatedReturnFromSlip.toFixed(2));
            },
        );
        ...
        function comboPackExec() {
            var allComboCard = $(".all-bs-card")
                .find(".single-bs-card.card-combo")
                .not(".hidden");
        ...
        function ComboRatioTotalMultipling() {
            ...
            $(".bet-slip-calculation")
                .find(".total-est-return")
                .text(afterMultiplion.toFixed(2));
        }
        ...
        comboCardBtn.on("click", function () {
            $(".bet-slip-calculation")
                .find(".total-est-return")
                .text(onlyComboRatioTotal.toFixed(2));
            resultNumb = 1;
            $(".inc-dec-bet").find(".result-num").text(resultNumb);
            comboBtnisOn = true;
            systemBTNisON = false;
